...
savepath = fullfile([cur_p '4_Synchronization_SMC\Results of CC curve\']);
...
for q=1:(size(a,2)-cycle+1)
    ...
    %需要把元素列到同一行输出
%     figure(),plot(R_value_rolling1(:,q));
%     grid on
% 
%     xlabel('Distance(um)','Fontsize',12)
% 
%     ylabel('R_value','Fontsize',12)
% 
%     saveas(gca,[savepath,'Rolling_R_value_gaussian5_',num2str(q),'.tif']);
end 
