...
buildingDir = fullfile([cur_p '2_Vasomotion_index\Raw_data\Test_radius_data.xlsx']);
savepath = fullfile([cur_p '2_Vasomotion_index\data_deal_radius\']);
...
select_enddata=[];
%delete the data of deltaR/R0<0.6%
jj=1;
for tt=1:length(select_enddata1)
    if select_enddata1(tt)>0.006
...
ylabel('detaR/R0')
hold off
saveas(gca,[savepath,'deltaR_R0_line',num2str(iii),'.tif']);
...
mean_deltaR(iii)=mean(bright_value);
interval=[];
amount=[];
bright_value=[];
bright_value1=[];
...
select_enddata1=[];
num=[];
A = [{'R','R0','deltaR/R0','peak_position','delta_radius_value','peak_deltaR/R0'}; num2cell(output)];
xlswrite([savepath,'single_data'],A,['cell_',num2str(iii),'']);
end
total_data=[frequency' average_amplitude' interval_mean' interval_SD' mean_deltaR' mean_diameter'];
B = [{'Frequency(Hz)','Average_deltaR/R0','interval_mean(s)','interval_SD','mean_deltaR' 'mean_radius'}; num2cell(total_data)];
