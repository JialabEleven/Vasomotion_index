...
buildingDir = fullfile([cur_p '3_Frequency_calculation\Test_calcium_data.xlsx']);
savepath = fullfile([cur_p '3_Frequency_calculation\data_frequency_calcium\']);

data=xlsread(buildingDir);

time=data(:,1); %因为我们之前是输入了一个25000*2的矩阵，但为了方便，我们可以按批次上传，先上传time的，再上传amplitude的。原理一样，在workspace分别得到两个文件，每个都是25000*1的数字矩阵。
...
title('低通滤波后频域幅度特性');
saveas(gca,[savepath,'frequency_domain_revise',num2str(i),'.tif']);
...
%         saveas(gca,[savepath,'compare_frequency_domain.tif']);
        ...
        saveas(gca,[savepath,'compare_frequency_domain_filter3+2.tif']);
        k=1;
...
%      saveas(gca,[savepath,'compare_pair',num2str(j),'.tif']);
    ...
    ylabel('Frequency-domain','Fontsize',12)
    legend('Before')
    saveas(gca,[savepath,'compare_pair_smooth3+2_',num2str(j),'.tif']);
...
xlswrite([savepath,'Total_data_filter'],output_frequency_filter_revise_smooth);%%%%