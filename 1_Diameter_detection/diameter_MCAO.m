...
for m=1:columns
    ...
    if boundary1(i,m)<15 %base line of right
        markright=m+right;
        break;
    end
end
...
for q=1:columns
    ...
    if boundary2(i,q)<15  %base line of left
        markleft=left-q;
        break;
    end
end
...
%mm=0;
if i>=2
    if diameter_cubic(i)> (1+0.06)*diameter_cubic(i-1)
        diameter_cubic(i)=(1+0.006)*diameter_cubic(i-1);
